...
# Corrigir as referências cruzadas de seções
...
Vamos inspecionar as colunas individuais e relacioná-las com os elementos da Figura 5.6:
...
Podemos responder a esta pergunta realizando o último dos três passos comuns em uma ADE: criando visualizações de dados. Vamos visualizar a distribuição da nossa variável de resultado $y = lifeexp$ na @fig-57.
...
gapminder2007 |> 
  ...
  labs(
    ...
    y = "Número de países",
    title = "Histograma da distribuição da expectativa de vida mundial"
  )
```
...
Para resumir, os cinco valores na coluna estimativa na @tbl-58 correspondem a linha de base para comparação da África (o intercepto) e também quatro desvios desse linha de base para os quatro continentes restantes: as Américas, Ásia, Europa e Oceania.

Você pode estar se perguntando por que a África foi escolhida como o grupo de linha de base para comparação. Isso acontece simplesmente porque ela vem primeiro em ordem alfabética dos cinco continentes; por padrão, o R organiza fatores/variáveis categóricas em ordem alfanumérica. Você pode mudar esse grupo de baseline para ser outro continente se manipular os “níveis” do fator da variável continente usando o pacote [forcats](https://forcats.tidyverse.org/). Veja o Capítulo 16 de [R for Data Science](https://r4ds.hadley.nz/) [@wickham2023rdata] para exemplos.

Agora, vamos escrever a equação para os nossos valores ajustados $\hat{y} = \widehat{life exp}$.

$$
\begin{align*}
\hat{y} = \widehat{\text{life exp}} &= b_0 + b_{\text{Amer}} \cdot 1_{\text{Amer}}(x) + b_{\text{Asia}} \cdot 1_{\text{Asia}}(x) + \\ 
&\quad + b_{\text{Euro}} \cdot 1_{\text{Euro}}(x) + b_{\text{Ocean}} \cdot 1_{\text{Ocean}}(x)\\ 
&= 54.8 + 18.8 \cdot 1_{\text{Amer}}(x) + 15.9 \cdot 1_{\text{Asia}}(x) + \\
&\quad + 22.8 \cdot 1_{\text{Euro}}(x) + 25.9 \cdot 1_{\text{Ocean}}(x)
\end{align*}
$$
...
No contexto de modelagem estatística, isso também é conhecido como uma variável dummy. No nosso caso, considere a primeira variável indicadora $1_{Amer}(x)$. Esta função indicadora retorna 1 se um país está nas Américas, 0 caso contrário:

$$
\begin{equation*}
\mathbb1_{\text{Amer}}(x) = \left\{
\begin{aligned}
...
Vamos juntar tudo isso e calcular o valor ajustado $\hat{y} = \widehat{life exp}$ para um país na África. Como o país está na África, todas as quatro funções indicadoras $1_{Amer}(x)$, $1_{Asia}(x)$, $1_{Euro}(x)$ e $1_{Ocean}(x)$ serão iguais a 0, e, portanto:

1_{\text{Amer}}(x)

$$
\begin{align*}
\widehat{\text{life exp}} &= b_0 + b_{\text{Amer}} \cdot \mathbb1_{\text{Amer}}(x) + b_{\text{Asia}} \cdot \mathbb1_{\text{Asia}}(x) + \\ 
&\quad + b_{\text{Euro}} \cdot \mathbb1_{\text{Euro}}(x) + b_{\text{Ocean}} \cdot \mathbb1_{\text{Ocean}}(x) \\ 
&= 54.8 + 18.8 \cdot \mathbb1_{\text{Amer}}(x) + 15.9 \cdot \mathbb1_{\text{Asia}}(x) + \\ 
&\quad + 22.8 \cdot \mathbb1_{\text{Euro}}(x) + 25.9 \cdot \mathbb1_{\text{Ocean}}(x) \\ 
...
$$
\begin{align*}
\widehat{\text{life exp}} &= 54.8 + 18.8 \cdot \mathbb1_{\text{Amer}}(x) + 15.9 \cdot \mathbb1_{\text{Asia}}(x) + 22.8 \cdot \mathbb1_{\text{Euro}}(x) \\
&\quad + 25.9 \cdot \mathbb1_{\text{Ocean}}(x) \\
