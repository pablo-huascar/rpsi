...
No Capítulo 5 sobre regressão básica, consideraremos apenas modelos com uma única variável explicativa $x$. Na @sec-5.1, a variável explicativa será numérica. Esse cenário é conhecido como regressão linear simples. Na @sec-5.2, a variável explicativa será categórica.
...
[^2]: Note que o símbolo $\cdot$ é equivalente ao símbolo matemático "multiplicar por" $\times$. Usaremos o símbolo $\cdot$ no restante deste livro, pois ele é mais sucinto.
...
-   `ordered`: Isso nos informa se a variável categórica é “ordinal”: se existe uma hierarquia codificada (como baixo, médio, alto). Neste caso, `continent` não é ordinal
...
Vamos nos concentrar novamente nos valores nas colunas termo e estimativa da @tbl-58. Por que há agora 5 linhas? Vamos analisá-las uma a uma:
